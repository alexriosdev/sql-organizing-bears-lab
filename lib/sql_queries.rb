...
def selects_most_prominent_color_and_returns_with_count
  "SELECT bears.color, COUNT(bears.color) FROM bears GROUP BY color ORDER BY COUNT(bears.color) DESC LIMIT 1;"
end

def counts_number_of_bears_with_goofy_temperaments
  "SELECT COUNT(bears.temperament) FROM bears GROUP BY temperament HAVING temperament = 'goofy';"
end
